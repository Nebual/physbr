...
import math, random, zlib #Zombie Libraries
...
class Map:
	def __init__(self,name="ATestForTheHardOfThinking"):
		...
		for v in self.leveldata["Entities"]:
			Entity(self,v["x"],v["y"],v["model"],ang=v.get("ang",0),gravity=v.get("gravity",True),wiredetails=v.get("wiredetails",{}))
			#ent = Entity(G.CurrentMap,x,y,model,gravity,wiredetails)
		for v in self.leveldata["TriggerEntities"]:
			WireEntity(self,v["modelclass"],wiredetails=v.get("wiredetails",{}))
		for k,ent in self.WireEntities.items():
			if ent.OutputNames:
				#print "Well",ent.WireName, ent.id,ent.OutputNames
				for wirename in ent.OutputNames:
					ent.Outputs.append(self.WireEntities[wirename])
					print "Wiring",ent.modelstring,"("+str(ent.id)+") to",self.WireEntities[wirename].modelstring
...
while True:
	...
	for event in pygame.event.get():
		if event.type == pygame.KEYDOWN:
			...
			if ekey == pygame.K_e and not ply.Holding: #Pick up stuff.
				...
				ply.vely = ply.Speed*-2 * G.CurrentMap.Gravity[1]
				ply.velx = ply.Speed*-2 * G.CurrentMap.Gravity[0]
	...
	if oldposx != ply.pos.x:
		...
		if ply.Stationary:
			...
			ply.UpsideDown = G.CurrentMap.Gravity[1] < 0 and 10 or 0
			ply.curframe = 3
			ply.model = ply.models[ply.curframe + ply.FacingLeft + ply.UpsideDown]
		elif G.Tick % 4 == 0:
			ply.curframe += 1
		...
		ply.Stationary = True
		ply.curframe=2
		ply.model = ply.models[2 + ply.FacingLeft + ply.UpsideDown]
	...
	#		oldCenter = ent.pos.center
	#		ent.model = pygame.transform.rotate(ent.model0,ent.ang)
			#PILImage.frombuffer("RGBA",
			#img = pygame.image.frombuffer(blend.tostring(), blend.size, blend.mode)

			temp = PILImage.fromstring ("RGBA", ent.model0.get_size (), pygame.image.tostring (ent.model0, "RGBA")).rotate(ent.ang,PILImage.BICUBIC)
			#tu = (0,0, ent.model0.get_size()[0]-1, ent.model0.get_size () [1] - 1)
			#temp = temp.transform (size2, Image.EXTENT,  tu, Image.BICUBIC)
			#temp = temp.rotate(ent.ang)
			ent.model = pygame.image.fromstring (temp.tostring(), temp.size, temp.mode)  
	#		ent.model.get_rect().center = oldCenter
